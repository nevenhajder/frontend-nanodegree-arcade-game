...
APP.Enemy.prototype.randX = function() {
    this.x = APP.constants.canvasWidth - (APP.constants.columnWidth * Math.floor(Math.random() * 6));
};

/* Choose a random row and position the Enemy */
APP.Enemy.prototype.randY = function() {
    this.y = APP.constants.enemyBaseY + (APP.constants.rowHeight * Math.floor(Math.random() * 3));
};

/* Choose a random speed and set it on the Enemy */
APP.Enemy.prototype.randSpeed = function() {
    this.speed = APP.constants.enemyBaseSpeed - Math.floor(APP.constants.enemySpeedFactor * (Math.random() * 3));
};
...
APP.Player.prototype.checkCollisions = function(enemies) {
    ...
    enemies.forEach(function(en) {
        if ((self.x < en.x + en.width && self.x + self.width > en.x) &&
            (self.y < en.y + en.height && self.y + self.height > en.y)
        ) {
            self.reset();
            self.lives--;
            self.checkAlive();
